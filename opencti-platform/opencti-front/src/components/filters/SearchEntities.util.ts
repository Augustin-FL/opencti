import useSearchEntities, { SearchEntitiesProps } from "../../utils/filters/useSearchEntities";
import {Dispatch, SyntheticEvent} from "react";
import {OptionValue} from "@components/common/lists/FilterAutocomplete";

let searchEntitiesScope: SearchEntitiesProps | undefined = undefined;

export namespace SearchEntitiesUtil {
  ...
  export const getOptionsFromEntities = (filterKey: string) => {
    ...
    const [entities, searchEntities] =
      useSearchEntities(searchEntitiesScope) as [
  ...
  export const getOptions = (filterKey: string, entities) => {
    let options = [];
    const isStixObjectTypes = [
      'elementId',
      'fromId',
